...
def function_dataset(data_path: str, tokenizer: PreTrainedTokenizer, user_tag: str = "", assistant_tag: str = "", \
                    pos_tempalte: str = "", neg_tempalte: str = "", seed: int = 0) -> (list, list):
    random.seed(seed)
    ...
    for statement in true_statements:
        ...
        for idx in range(1, len(tokens) - 5):
            ...
            pos_statements.append(f"{user_tag} {pos_tempalte} {assistant_tag} " + truncated_statement)
            neg_statements.append(f"{user_tag} {neg_tempalte} {assistant_tag} " + truncated_statement)

    # Create training data
    ntrain = 512
    combined_data = [[honest, untruthful] for honest, untruthful in zip(pos_statements, neg_statements)]
    ...
    # Create test data
    reshaped_data = np.array([[honest, untruthful] for honest, untruthful in zip(pos_statements[:-1], neg_statements[1:])]).flatten()
...
def plot_detection_results(input_ids, rep_reader_scores_dict, THRESHOLD, start_answer_token=":"):
    ...
    for rep, s_style, n_style in zip(selected_concepts, selection_style, norm_style):
        ...
        start_tok = 0
        for i in range(len(words)-len(start_answer_token)+1):
            if words[i:i+len(start_answer_token)] == start_answer_token:
                start_tok = i + len(start_answer_token) - 1
...
def plot_lat_scans(input_ids, rep_reader_scores_dict, layer_slice, assistant_token, max_len=40):
    for rep, scores in rep_reader_scores_dict.items():
        ...
        for i in range(len(input_ids)-len(assistant_token)+1):
            if input_ids[i:i+len(assistant_token)] == assistant_token:
                start_tok = i + len(assistant_token) - 1
        print(f"index of generation start token: {start_tok}")
        print(f"shape of scores: {np.array(scores).shape}")
        standardized_scores = np.array(scores)[start_tok:start_tok+max_len,layer_slice]
